...
public class GridSimulation extends Simulation {
	...
	/**
	 * this method calculates and makes movements of the cows. The current score will be calculated for
	 * the team that caught the cow.
	 * 
	 */
	private void cowSimulation() {
		/*
		 * movingcows vector gathers all of the cows that are able to move in this
		 * simulation step. A cow can only move after some simulation steps. This depends on the cowSpeed.
		 */
	 ...
	 * switch of a fence, the fence will be opened. In the other case it will be closed.
	 * While closing of a fence all agents and cows will be pushed
	 * back to their old positions if they are on the same cell of fence cell.
	 * 
	...
	private boolean canOpen(int x, int y, String direction) {
		try {
			if (direction.equalsIgnoreCase("up")
					&& (state.board[x - 1][y].agent || state.board[x + 1][y].agent || state.board[x][y + 1].agent)) {
				return true;
			} else if (direction.equalsIgnoreCase("down")
					&& (state.board[x - 1][y].agent || state.board[x + 1][y].agent || state.board[x][y - 1].agent)) {
				return true;
			} else if (direction.equalsIgnoreCase("left")
					&& (state.board[x][y + 1].agent || state.board[x][y - 1].agent || state.board[x + 1][y].agent)) {
				return true;
			} else if (direction.equalsIgnoreCase("right")
					&& (state.board[x - 1][y].agent || state.board[x][y + 1].agent || state.board[x][y - 1].agent)) {
				return true;
			}
	...
	public void postSimulationStep() {
		...
		state.averageScore[0] = ((double) state.cowsSum[0]) / ((double) state.currentStep);
		state.cowsSum[1] += state.cowsInCorral[1];
		state.averageScore[1] = ((double) state.cowsSum[1]) / ((double) state.currentStep);
