...
import massim.simulationconnector.FlashXMLVisualization;

import org.w3c.dom.Document;
import org.w3c.dom.Element;

public class GridFlashXMLVitualization extends FlashXMLVisualization{

	public GridFlashXMLVitualization(String server, int port, String service) {
		super(server, port, service);
	}
	public GridFlashXMLVitualization(){}
	...
	//	Element el_root = doc.getDocumentElement();
		...
		for(int i = 0 ; i< worldState.sizex; i++){
			for(int j = 0; j < worldState.sizey;j++){
				 ...
				 if(worldState.board[i][j].obstacle){
				 ...
				 if(worldState.board[i][j].stable1){
					Element el = doc.createElement("stable1");
					el.setAttribute("posx", ""+i);
					el.setAttribute("posy", ""+j);
					el_state.appendChild(el);
				}
				 if(worldState.board[i][j].stable2){
